...
# generate motion.json file for the motion files in the BIDS dataset
def generate_motion_json_file(required_fields: dict, optional_fields: dict = {}):
    """
    Generate a motion.json file based on the provided schema.
    ...
    required_fields (dict): A dictionary containing the required fields and their values.
                            ...
                            - "SamplingFrequency" (float): Sampling frequency of the motion data.
    ...
    """
    required_keys = ["TaskName", "SamplingFrequency"]
    ...
    motion_data = {**required_fields, **optional_fields}
    
    # Return the motion data as a dict
    return motion_data

# generate a channels.tsv file for the motion files in the BIDS dataset
def generate_channels_tsv(tracked_markers: list, sampling_rate: int = 100):
    """
    ...
    """
    if not isinstance(tracked_markers, list) or not all(isinstance(marker, str) for marker in tracked_markers):
        raise ValueError("tracked_markers should be a list of strings")
    if not all("_" not in marker for marker in tracked_markers):
